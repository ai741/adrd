...
def tongyongLinf(x,y,epsilon):
    final=x+y
    final = batch_l1_proj(final.cpu(),epsilon)
    return final
...
def perturb_iterative(xvar, yvar, predict, nb_iter, eps, eps_iter, loss_fn,
    ...
    if mask is None:
        ...
        final_delta = x_adv - xvar
    else:
        x_adv = clamp(xvar + delta * mask, clip_min, clip_max)
        final_delta = x_adv - xvar
    return x_adv, final_delta
...
class PGDAttack(Attack, LabelMixin):
    ...
    def perturb(self, x, y=None, mask=None):
        ...
        rval,scond_data = perturb_iterative(
