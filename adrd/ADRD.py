...
from Masked_PGD import LinfPGDAttack, tongyongLinf
from ADRD_utils import parse_args, load_mask, closing_resize, load_image_from_path
...
def init(epsilon: int = 3, steps: int = 100, alpha: int = 1,
    ...
    if ckpt is None:
       ckpt = 'ldm/models/sd-v1-4/sd-v1-4.ckpt'
...
def infer(img: PIL.Image.Image, config, tar_img: PIL.Image.Image = None, mask: PIL.Image.Image = None) -> np.ndarray:
    ...
    attack_output,final_delta = attack.perturb(data_source, label, mask=mask)
...
if __name__ == "__main__":
    ...
    target_image_path = 'adrd.png'
    bls = input_size//block_num
    first_data = torch.zeros((3, input_size, input_size), device=device)
    cc_data=[]
        ...
        for img_id in os.listdir(image_dir_path):
            ...
            for i in tqdm(range(block_num)):
                for j in tqdm(range(block_num)):
                    ...
                    output_image[bls_w*i: bls_w*i+bls_w, bls_h*j: bls_h*j + bls_h],second_data = infer(img_block, config, tar_block, input_mask)
            final_data = tongyongLinf(first_data,second_data,epsilon=epsilon)
            cc_data.append(second_data)
    ...
    print(f"Final data saved to {args.save_path}")
    # torch.save(cc_data, r'E:\mist-main\ckpt\cc_data.pt')
